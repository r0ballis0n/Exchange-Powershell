...
#Commands
foreach ($adgroup1mailbox in $adgroup2mailboxes) {Set-MailboxFolderPermission -Identity $adgroup1mailbox":\Calendar" -User $User -AccessRights Editor}
foreach ($adgroup2mailbox in $adgroup2mailboxes ) {Set-MailboxFolderPermission -Identity $adgroup2mailbox":\Calendar" -User $User -AccessRights Editor}
